import { Terminal, FileCode, Folder, Share2, CheckCircle, Lightbulb, ExternalLink } from "lucide-react";
        ...
        <div className="space-y-12">
            <div>
                ...
                <p className="text-lg text-slate-400 leading-relaxed max-w-2xl">
                    The <code className="text-rose-400 bg-black/30 px-1.5 py-0.5 rounded">packages/shared</code> package contains
                    validation schemas and types shared between the API and Web apps. This is the key to end-to-end type safety.
                </p>
            </div>
            ...
            <section className="space-y-4">
                ...
                <div className="grid gap-4 md:grid-cols-3">
                    <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4">
                        ...
                        <p className="text-slate-400 text-sm">Same validation on frontend and backend. Catch errors at compile time.</p>
                    </div>
                    <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4">
                        <CheckCircle className="w-6 h-6 text-green-400 mb-2" />
                        <h3 className="font-bold text-white mb-1">Single Source of Truth</h3>
                        <p className="text-slate-400 text-sm">Define schemas once, import everywhere. No drift between API and UI.</p>
                    </div>
                    <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4">
                        <CheckCircle className="w-6 h-6 text-green-400 mb-2" />
                        <h3 className="font-bold text-white mb-1">Lightweight</h3>
                        <p className="text-slate-400 text-sm">Valibot is 10x smaller than Zod. Perfect for frontend validation.</p>
                    </div>
                </div>
            </section>
...
└── package.json`}
            ...
            <section className="space-y-4">
                ...
                <p className="text-slate-400">
                    Use <a href="https://valibot.dev" target="_blank" rel="noreferrer" className="text-orange-400 hover:underline">Valibot</a> to
                    define schemas with automatic TypeScript inference:
                </p>
                <CodeBlock
...
export type UpdateTodo = v.InferOutput<typeof UpdateTodoSchema>;`}
            ...
            <section className="space-y-4">
                ...
                <p className="text-slate-400">
                    Re-export everything from <code className="text-purple-400">src/index.ts</code> for clean imports:
                </p>
                <CodeBlock
...
function TodoForm({ onSubmit }: { onSubmit: (data: CreateTodo) => void }) {
    ...
    // ...
            ...
            <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-xl p-4 flex gap-3">
                ...
                <div>
                    <p className="text-indigo-300 text-sm font-medium mb-1">Pro Tip: Workspace Dependencies</p>
                    <p className="text-slate-400 text-sm">
                        The shared package uses <code className="bg-black/30 px-1 rounded">workspace:*</code> in package.json.
                        The monorepo automatically rebuilds it when source changes. Run <code className="bg-black/30 px-1 rounded">bun run dev</code>
            ...
            <section className="space-y-4">
                ...
                <div className="flex flex-wrap gap-3">
                    ...
                    <Link to="/docs/packages/web" className="px-4 py-2 bg-blue-500/20 border border-blue-500/30 rounded-lg text-blue-300 hover:bg-blue-500/30 transition-colors">
                        Web Package →
                    </Link>
                </div>
            </section>
