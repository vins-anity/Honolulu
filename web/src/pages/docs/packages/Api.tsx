import { Code2, Database, Folder, Layers, Terminal } from "lucide-react";
		...
		<div className="space-y-12">
			<div>
				...
				<p className="text-lg text-slate-400 leading-relaxed max-w-2xl">
					The <code className="text-rose-400 bg-black/30 px-1.5 py-0.5 rounded">apps/api</code>{" "}
					package is powered by{" "}
					<a
						href="https://hono.dev"
						target="_blank"
						rel="noreferrer"
					...
					—a lightweight, ultrafast web framework that works everywhere (Bun, Node, Cloudflare
					Workers, Deno).
				</p>
			</div>
			...
			<div className="bg-teal-500/10 border border-teal-500/20 rounded-xl p-4">
				<p className="text-teal-300 text-sm">
					...
					<a href="https://hono.dev/docs" target="_blank" rel="noreferrer" className="underline">
...
app.get("/posts/:id/comment/:commentId", async (c) => {
    ...
    // ...
				...
				<p className="text-slate-400 mb-3">
					The <code className="text-rose-400">c</code> object gives you access to request, response,
					and environment:
				</p>
...
export default app;`}
			...
			<section className="space-y-4">
				...
				<p className="text-slate-400">
					...
					with <code className="text-green-400">postgres.js</code> for type-safe SQL. Define your
					schema in <code className="text-green-400">db/schema.ts</code>:
				</p>
				<CodeBlock
...
SUPABASE_ANON_KEY=your-anon-key`}
			...
			<section className="space-y-4">
				...
				<div className="flex flex-wrap gap-3">
					...
					<Link
						to="/docs/deployment/cf-workers"
						className="px-4 py-2 bg-orange-500/20 border border-orange-500/30 rounded-lg text-orange-300 hover:bg-orange-500/30 transition-colors"
					>
						Cloudflare Workers →
					</Link>
				</div>
