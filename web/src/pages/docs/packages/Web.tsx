import { Code2, FileCode, Folder, Terminal, Zap } from "lucide-react";
		...
		<div className="space-y-12">
			<div>
				...
				<p className="text-lg text-slate-400 leading-relaxed max-w-2xl">
					The <code className="text-rose-400 bg-black/30 px-1.5 py-0.5 rounded">apps/web</code>{" "}
					package is the React frontend, built with{" "}
					<a
				...
				<div className="grid gap-4 md:grid-cols-2">
					<div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4">
						...
						<p className="text-slate-500 text-xs mt-2">
							Hot reload at <code>localhost:5173</code>
						</p>
					</div>
					<div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4">
    ...
    todos: {
        ...
        create: async (todo: CreateTodo): Promise<Todo> => {
			...
			<section className="space-y-4">
				...
				<p className="text-slate-400">
					Use the same Valibot schemas from <code className="text-yellow-400">shared</code> for
					frontend validation:
				</p>
				<CodeBlock
        ...
        onSuccess: () => {
			...
			<section className="space-y-4">
				...
				<p className="text-slate-400">
					...
					for simple, performant client-side state (UI state, user preferences, etc.):
				</p>
				<CodeBlock
            ...
            {sidebarOpen ? "Close" : "Open"} Menu
				...
				<div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-4">
					<p className="text-yellow-300 text-sm">
						<strong>Best Practice:</strong> Use Zustand for client-only state (UI, forms). Use
						TanStack Query for server state (API data). Don't mix them!
					</p>
				</div>
			</section>
			...
			<section className="space-y-4">
				...
				<div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4">
					<p className="text-amber-300 text-sm">
						<strong>Important:</strong> Vite exposes env vars that start with{" "}
						<code className="bg-black/30 px-1 rounded">VITE_</code> to client code. Never put
						secrets in <code>VITE_</code> prefixed variables!
					</p>
				</div>
			</section>
