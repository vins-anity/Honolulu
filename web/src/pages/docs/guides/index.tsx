import { Database as DbIcon, Lock, Settings } from "lucide-react";
		...
		<div className="space-y-12">
			...
			<section className="space-y-4">
				...
				<p className="text-slate-400">
					Create <code className="text-rose-400">.env</code> in <code>apps/api/</code>:
				</p>
				<CodeBlock
					filename=".env"
					language="bash"
...
BETTER_AUTH_SECRET=your-secret-key`}
			...
			<section className="space-y-4">
				...
				<p className="text-slate-400">
					Create <code className="text-blue-400">.env.local</code> in <code>apps/web/</code>:
				</p>
				<CodeBlock
					filename=".env.local"
					language="bash"
...
VITE_SUPABASE_ANON_KEY=your-anon-key`}
				...
				<div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4">
					<p className="text-amber-300 text-sm">
						<strong>⚠️ Warning:</strong> Vite exposes all{" "}
						<code className="bg-black/30 px-1 rounded">VITE_</code> prefixed variables to the client
						bundle. Never put secrets in these variables!
					</p>
				</div>
			</section>
...
await supabase.auth.signInWithPassword({
    ...
    password: "password",
			...
			<section className="space-y-4">
				...
				<p className="text-slate-400">Alternative option for self-hosted authentication.</p>
