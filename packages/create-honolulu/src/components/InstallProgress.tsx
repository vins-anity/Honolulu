...
export const InstallProgress: React.FC<InstallProgressProps> = ({ status, currentStep }) => {
	const renderStepIcon = (stepStatus: "pending" | "running" | "done" | "error" | "skipped") => {
		switch (stepStatus) {
			case "running":
	...
	return (
		<Box flexDirection="column" marginTop={1}>
			<Box>
				...
				<Box marginLeft={1}>{renderStepText(status.scaffold, "🌺 Planting seeds...")}</Box>
			</Box>
			<Box>
				...
				<Box marginLeft={1}>
					{renderStepText(
						status.git,
						status.git === "skipped" ? "📦 Git initialization (skipped)" : "📦 Initializing git...",
					)}
				</Box>
			</Box>
			<Box>
