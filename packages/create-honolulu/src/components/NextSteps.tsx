...
export const NextSteps: React.FC<NextStepsProps> = ({
	...
	return (
		<Box flexDirection="column" marginTop={1}>
			...
			<Box marginTop={1} flexDirection="column">
				<Text color="cyan">✓ Database: {getDatabaseLabel(database)}</Text>
				<Text color="cyan">✓ Authentication: {getAuthLabel(auth)}</Text>
				<Text color="cyan">
				...
				<Text color="cyan">
					✓ Style:{" "}
					{style === "tailwind" ? "Tailwind CSS" : style === "shadcn" ? "Shadcn UI" : "Classic CSS"}
				</Text>
			</Box>

			{database === "supabase" || auth === "supabase" ? (
				<Box marginTop={1}>
					<Text color="yellow">⚠️ Configure Supabase credentials in .env file</Text>
				</Box>
			) : null}

			{database !== "sqlite" && database !== "supabase" && (
				<Box marginTop={1}>
					<Text color="yellow">⚠️ Set DATABASE_URL in .env file</Text>
				</Box>
			)}
		</Box>
	);
